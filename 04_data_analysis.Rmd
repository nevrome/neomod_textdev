...
```{r sed-dev-matrix-bt, echo = FALSE, fig.scap = "Plotmatrix der SEDs für \\textit{burial type}", fig.cap = "Matrix mit $8*8=64$ Einzelplots. Jeder Plot zeigt die Entwicklung der Squared Euclidian Distance (SED) für die Variable \\textit{burial type} über den Zeitraum von 2200 bis 800calBC für jeweils eine Regionenbeziehung. Über der grauen Linie der echten Datenentwicklung liegt jeweils eine stärkere in der Farbe der Spaltenregion, die den Verlauf eines dynamisch eingepassten Splines (loess, span = 0.3) zeigt.", out.extra = ""}
knitr::include_graphics("../neomod_analysis/figures_plots/sed/regions_regions_squared_euclidian_distance_burial_type.jpeg")
```

```{r sed-dev-bt, echo = FALSE, fig.scap = "Entwicklung der \\textit{burial type} SEDs für jede Region", fig.cap = "Alternative Darstellung der Plotmatrix in \\ref{fig:sed-dev-matrix-bt}.", out.extra = ""}
knitr::include_graphics("../neomod_analysis/figures_plots/sed/regions_squared_euclidian_distance_burial_type.jpeg")
```

```{r sed-dev-matrix-bc, echo = FALSE, fig.scap = "Plotmatrix der SEDs für \\textit{burial construction}", fig.cap = "Wie Abbildung \\ref{fig:sed-dev-matrix-bt}.", out.extra = ""}
...
```{r sed-dev-bc, echo = FALSE, fig.scap = "Entwicklung der \\textit{burial construction} SEDs für jede Region", fig.cap = "Alternative Darstellung der Plotmatrix in \\ref{fig:sed-dev-matrix-bc}.", out.extra = ""}
...
```{r sed-mean-matrix-bt, echo = FALSE, fig.scap = "Matrix der durchschnittlichen SEDs für \\textit{burial type}", fig.cap = "Die Farbintensität bildet die Werteverteilung ab.", out.extra = ""}
...
```{r sed-mean-matrix-bc, echo = FALSE, fig.scap = "Matrix der durchschnittlichen SEDs für \\textit{burial construction}", fig.cap = "Wie Abbildung \\ref{fig:sed-dev-matrix-bc}.", out.extra = ""}
...
\begin{landscape}
```{r mantel-bt-bc, echo = FALSE, fig.scap = "\\textit{burial type} SED aufgetragen gegen \\textit{burial construction} SED für verschiedene Zeitfenster", fig.cap = "huhu", out.extra = ""}
...
```{r map-regions-distance-classes, echo = FALSE, fig.scap = "Karte des klassifizierten, räumlichen Distanznetzwerks", fig.cap = "Die Kantenstärke repräsentiert das Kantengewicht: Breite Linien repräsentierten enge Beziehungen mit niedrigem Distanz-Wert. Die Kanten des Netzwerks sind bogenförmig um Überschneidungungen zu vermeiden, die es erschweren würden das Gewicht der einzelnen Verbindungen abzulesen.", out.extra = ""}
