...
Module erweitert einen Eingabedatensatz sukzessive um generierte Daten
...
einem hypothetischen Nullzeitpunkt. Der Eingabedatensatz muss in Form
eines \texttt{data.frames}, also der in R üblichen Datenstruktur für
tabellierte Daten unterschiedlicher Typen, vorliegen. Jede Zeile in
diesem Datensatz enthält zunächst die Parameter -- später auch die
Ausgabedaten -- für eine Population, ihre Relationen und Ideen. Komplexe
Parameter sind dabei in \texttt{list\ columns} gespeichert. Diese
erlauben es, fast beliebige Datenstrukturen in den Zellen eines
...
\addlinespace \hline \addlinespace
\textit{Populationsparamter}\newline \textbf{unit\_names}\newline \textit{list of characters or factors} & Eine Liste der Gruppennamen, wahlweise mit einer Definition ihrer inneren Reihenfolge. & Liste mit Namen. Die Anzahl muss \textit{unit\_amount} entsprechen.\newline     \texttt{list(factor("regionA", levels = regionen), factor("regionB", levels = regionen))}\\
\addlinespace \hline \addlinespace
...
\texttt{init\_population\_settings} dient dazu, eine Abfolge von
Individuen zu generieren, die nur durch ihre Lebenszeit und ihre
Gruppenzugehörigkeit definiert sind. Menschen sind -- wie in Kapitel
@ref\{simulation-theorie\} beschrieben -- sehr einfach modelliert.

\hypertarget{expansionssimulation}{%
\subsubsection{Expansionssimulation}\label{expansionssimulation}}

\hypertarget{wertebereiche-der-simulationsparameter}{%
\subsection{Wertebereiche der
Simulationsparameter}\label{wertebereiche-der-simulationsparameter}}
