...
Submodule gegliedert: \texttt{init\_population\_settings},
\texttt{init\_relations\_settings} und \texttt{init\_ideas\_settings},
die nacheinander aufgerufen werden können -- die Interfacefunktion
...
\texttt{init\_population\_settings} dient dazu eine große Menge von
...
\texttt{init\_population\_settings} werden die Gruppenpopulationen zur
...
Das Modul \texttt{init\_relations\_settings} erweitert diesen von
\texttt{init\_population\_settings} modifizierten Eingabedatensatz. Es
dient dazu, die vorhandene Gesamtpopulation sinnvoll inneinander zu
verknüpfen um ein diachrones, soziales Netzwerk zu schaffen. Dafür wird
zunächst für jedes Modell ein Objekt der Konfigurationsklasse
\texttt{relations\_settings} instanziiert, das neben der in
\texttt{init\_relations\_settings} generierten Population auch die
...
dreigliedriger Prozess: Vertikale und Horizontale Beziehungen, die sich
...
somit die Gesamtpopulation verschränkt. Die Erzeugung der vertikalen
Beziehungen mit \texttt{generate\_vertical\_relations} funktioniert also
gruppenweise, verbindet ein jüngeres Individuum mit -- wenn im
entsprechenden Zeitfenster vorhanden -- zwei älteren und orientiert sich
dabei nicht an deren realem Alter sondern an Indexdistanzen (für eine
Erklärung der Hintergründe dieser Lösung siehe Kapitel
...
Altersdistanzfenster dafür wurde zwischen 40 und 15 Jahren festgelegt.
Die zufällige Auswahl der Eltern erfolgt also in einem Pool von
...
unterschreiten. Die Erzeugung der
\texttt{generate\_horizontal\_relations} \ldots{}
