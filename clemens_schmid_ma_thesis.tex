...
% \AtBeginFigures{\addcontentsline{toc}{chapter}{Abbildungen}}
...
der Kalibrationskurve (71-46401calBP). Zur Kalibration wurde das R Paket
Bchron\footnote{\textcite{haslett_simple_2008}} und die darin
...
800-2200calBC (1401 Jahre) vorweisen können. Die Fragestellung erfordert
es auch, alle Daten ohne Raumbezug, also ohne Koordinateninformation, zu
entfernen. Radon-B stellt in der Variable \emph{Feature Type}
(\emph{sitetype} in c14bazAAR) teilweise kategorisierte Informationen
zum Befundkontext jedes Datums zur Verfügung: Fragestellungsrelevant
sind die Kategorien \emph{cemetery}, \emph{Grave}, \emph{Grave (mound)},
\emph{Grave (mound) inhumation}, \emph{Grave (mound) cremation},
\emph{Grave (flat)}, \emph{Grave (flat) inhumation}, \emph{Grave (flat)
cremation}, \emph{Grave (cremation)} und \emph{Grave (inhumation)}.
...
mit \ldots{} Eine Karte der so vorbereiteten Gräberdaten zeigt die hohe
Heterogenität der Datendichte in verschiedenen Regionen Europas (s
\ref{fig:general-map-research-area-unzoomed}.

\begin{figure}
\includegraphics{../neomod_analysis/figures_plots/general_maps/general_map_research_area_unzoomed} \caption[Übersichtskarte \textsuperscript{14}C Daten an bronzezeitlichen Gräbern]{Karte der \textsuperscript{14}C-Daten des Radon-B Datensatzes in Europa aus einem Zeitfenster von 2200-800calBC. Daten außerhalb des durch die gewählten Kartengrenzen definierten Areals werden ignoriert. Jedes Datum ist nach seinen Kontextinformationen hinsichtlich der Variablen Burial type und Burial construction in Form und Farbe markiert. Der rote Rahmen markiert das künstlich festgelegte Untersuchungsareal.}\label{fig:general-map-research-area-unzoomed}
\end{figure}

Das Untersuchungsareal dieser Arbeit folgt keinen natürlichen oder
kulturellen Grenzen, sondern wurde rein künstlich auf Grundlage der
räumlichen Verteilung der zusammengestellten \textsuperscript{14}C-Daten
festgelegt. Auf Grundlage visueller Analyse der Punktdichte wurde ein
Rechteck aufgespannt (siehe Abbildung Y). Die Projektion, die dieser
Festlegung, allen Kartierungen und auch der Regionengliederung zugrunde
liegt ist bewusst mit EPSG:102013\footnote{\url{https://epsg.io/102013}
...
vergleichbaren, räumlichen Untersuchungseinheiten.
...
\textbf{burial\_type} und \textbf{burial\_construction} in sinnvollen
und der verfügbaren Datenmenge angemessenen Einheiten beobachten zu
können (siehe Abbildung Z). Der Prozess der Erstellung dieser Regionen
war semiautomatisch und darauf angelegt kulturelle Makroregionen der
Bronzezeit zumindest näherungsweise abzubilden. Dafür stand mir auch
eine unpublizierte, händisch entworfene Regionengliederung von Jutta
Kneisel und Oliver Nakoinz als Vorlage zur Verfügung. In den Grenzen des
...
das effektive Untersuchungsareal nicht noch weiter verkleinern zu müssen
(siehe Abbilung A für eine Aufstellung der absoluten Menge an Daten pro
Region).
