...
forcats\footnote{\textcite{wickham_forcats_2018}}, kableExtra\footnote{\textcite{zhu_kableextra_2018}},
pbapply\footnote{\textcite{solymos_pbapply_2018}}, plyr\footnote{\textcite{wickham_split-apply-combine_2011}},
...
inkonsistent oder fehlerhaft (siehe Kapitel \ref{source-criticism}).
...
800-2200calBC (1401 Jahre) vorweisen können (7.543 Daten). Die
Fragestellung erfordert es auch, alle Daten ohne Raumbezug, also ohne
Koordinateninformation, zu entfernen (7,266 Daten). Radon-B stellt in
der Variable \emph{Feature Type} (\emph{sitetype} in c14bazAAR)
...
\emph{Grave (cremation)} und \emph{Grave (inhumation)} (2.336 Daten).
...
mit den Kategorien \emph{mound}, \emph{flat} und \emph{unknown}. Nach
diesem Arbeitsschritt lässt sich der Hauptausgangsdatensatz als Tabelle
mit 2.366 Zeilen und 13 Spalten beschreiben, darunter die hier
wesentlichen mit Angaben zu Labornummer, Koordinaten, Dichteverteilung
des kalibrierten Alters und \emph{burial\_type} sowie
...
Der Arbeitsdatensatz, der nach oben beschriebenem Vorgehen
zusammengestellt wurde, umfasst also 1.894 Einträge aus der Radon-B
Datenbank. Die effektive Anzahl an \textsuperscript{14}C Daten, die
diese Einträge wiedergeben, ist geringer: Eine Zählung der Labornummern
ergibt XXX individuelle Werte. Diese Diskrepanz ergibt sich aus
Einträgen mit keiner (\emph{n/a-n/a}) oder unvollständiger (z.B.
\emph{MAMS-n/a}, \emph{Gd-n/a}, \emph{Ke-n/a}) Labornummer sowie Daten
die mehrfach in die Datenbank eingegeben wurden (z.B. \emph{OxA-29003},
\emph{GrN-10754}, \emph{BRAMS-1217}). Letzteres betrifft XXX Einträge,
die ein und dasselbe \textsuperscript{14}C Datum zwei- oder mehrfach
repräsentieren. Die Anzahl an Gräbern, die durch die Einträge
repräsentiert werden ist noch geringer: Für XXX Einträge gilt, dass die
ihnen zugehörige Kombination aus Fundplatz und Befund von mindestens
einem weiteren Eintrag abgedeckt wird. Der wichtigste Grund dafür ist,
dass für ein Grab häufig mehrere \textsuperscript{14}C Daten in Auftrag
gegeben werden.

Die Abweichungen zwischen Eintrag und \textsuperscript{14}C Daten sowie
\textsuperscript{14}C Daten und Gräbern sind schwerwiegend und sollten
korrigiert werden. Davon wurde für diese Arbeit jedoch aus verschiedenen
Gründen abgesehen. Erstere Abweichung ist durch unvollständige
Datenpublikation und Fehleingabe bedingt und kann mit der stückweisen
Verbesserung des Radon-B Datensatzes gelöst werden. Eine systematische
Lösung im Analysecode, etwa das schlichte Zusammenführen der doppelten
Einträge, ist nicht einfach möglich, da die Einträge sich in den
Variablen jenseits der Labornummer häufig unterscheiden. Hier wäre also
eine Prüfung auf Korrektheit erforderlich, um den besseren der Einträge
zu finden. Von einer Einzellfalldiskussion wird jedoch bewusst abgesehen
um die reproduzierbare Natur dieser Arbeit nicht in Frage zu stellen. Es
ist außerdem davon auszugehen, dass der Einfluss dieser Abweichung auf
das Gesamtergebnis angesichts der ohnehin approximativen Ergebnisse
gering ausfällt. Die zweite Abweichung ist schwerwiegender, da sie
immerhin nahezu ein Drittel der Einträge betrifft und keine Aussicht
besteht, dass sich dieses Problem mit einer Verbesserung der Datenlage
lösen wird. Von einer Korrektur wurde an dieser Stelle aus zwei Gründen
abgesehen: Zunächst ist die Befundangabe für manche Einträge sehr
unpräzise (\emph{Kollektivgrab}, \emph{Einzelgrab}, \emph{from ring
ditch}) und es ist nicht ersichtlich, ob die Daten tatsächlich von einer
einzelnen Bestattung stammen. Weiterhin lässt die Verteilung dieser 514
Einträge hinsichtlich der wesentlichen Variablen \emph{burial type}
(cremation: XXX, inhumation: XXX, unknown: XXX) und \emph{burial
construction} (flat: XXX, mound: XXX, unknown: XXX) keine starken
systematischen Tendenzen erkennen. Es kann angenommen werden, dass
dieses Problem für die Fragestellung dieser Arbeit vernachlässigbar ist.
...
gewählt werden mussten. Nur aus den acht Regionen, die in Abbildung Z
...
Schwellwert dafür wurde mit 70 Gräbern jedoch sehr niedrig angelegt um
...
Einheiten orientiert, die die Kreise im wesentlichen einschließen. Ihre
im folgenden stets eingehaltene, geographische Reihenfolge von Südost
nach Nordwest soll die Lesbarkeit- und Interpretierbarkeit von
Abbildungen erhöhen: \emph{Austria and Czechia} (mit 93
\textsuperscript{14}C Daten aus dem Untersuchungszeitraum),
\emph{Poland} (153), \emph{Southern Germany} (232), \emph{Northeastern
France} (71), \emph{Northern Germany} (543), \emph{Southern Skandinavia}
(216), \emph{Benelux} (326) und \emph{Southern England} (115). Durch die
Regionengliederung verringerte sich das effektive Untersuchungsareal
weiter. Von den 1.894 Daten im Rechteckareal verblieben 1.749.
